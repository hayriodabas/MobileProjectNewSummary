...
import 'package:flutterfire/widgets/flutter_backdrop.dart';
...
class _FrontPanelState extends State<FrontPanel> {
          ...
          return Scaffold(
              ...
              RadialAxis( axisLineStyle: AxisLineStyle(thickness: _thickness,
               ...
               showFirstLabel: false,
                  minimum: 0,
                  maximum: 12, interval: 1,
                  startAngle: 270, endAngle: 270,
              pointers: <GaugePointer>[
                MarkerPointer(value: _firstMarkerValue,
                  onValueChanged: onFirstPointerValueChanged,
                  onValueChanging: onFirstPointerValueChanging,
                  enableDragging: true,  borderColor: const Color(0xFFFFCD60),
                  borderWidth: _borderWidth, color: Colors.white,
                  markerHeight:  _markerSize,
                  markerWidth: _markerSize, markerType: MarkerType.circle,
                ),
                MarkerPointer(value: _secondMarkerValue,
                  onValueChanged:   onSecondPointerValueChanged,
                  onValueChanging: onSecondPointerValueChanging,
                  color: Colors.white,
                  enableDragging: true,  borderColor: const Color(0xFFFFCD60),
                  markerHeight: _markerSize,   borderWidth: _borderWidth,
                  markerWidth: _markerSize, markerType: MarkerType.circle,
                ),

              ],
              ranges: <GaugeRange>[GaugeRange(endValue: _secondMarkerValue,
                  sizeUnit: GaugeSizeUnit.factor,
                  startValue: _firstMarkerValue, startWidth : _thickness, endWidth: _thickness)],
              annotations:<GaugeAnnotation>[
                GaugeAnnotation( widget:Row(
                  children: <Widget>[
                    ...
                    Text(' hr', style: TextStyle(fontSize:  _annotationFontSize,
                        fontFamily: 'Times',fontWeight: FontWeight.bold,
                        color:const Color(0xFF00A8B5)),),
                  Text(' $_minutesValue',
                  style: TextStyle(fontSize: _annotationFontSize,
                ...
                Text('m', style: TextStyle(fontSize:  _annotationFontSize,
                    fontFamily: 'Times',fontWeight: FontWeight.bold,
                    color:const Color(0xFF00A8B5)),)
                  ],
                ),
                    positionFactor: 0.1, angle: 0
                )]
          )
          ]
          ),
  ...
  void onFirstPointerValueChanged(double value){
    setState(() {
     ...
     final String _hourValue = '$_value';
      _annotationValue = _hourValue .length == 1 ? '0' + _hourValue  :
      _hourValue;
  ...
  void _calculateMinutes(int _value){
    final double _minutes = (_firstMarkerValue
        - _secondMarkerValue).abs() - _value;
     final List<String> _minsList = _minutes.toStringAsFixed(2).split('.');
     double _currentMinutes = double.parse(_minsList[1]);
     _currentMinutes = _currentMinutes > 60 ? _currentMinutes - 60 : _currentMinutes;
    final String _actualValue = _currentMinutes.toInt().toString();
    _minutesValue =_actualValue.length == 1 ? '0'+ _actualValue : _actualValue;
...
Widget getRadialSlider(bool isTileView) {
  ...
  return  SfRadialGauge(
      axes: <RadialAxis>[
        RadialAxis( axisLineStyle: AxisLineStyle(thickness: 0.06,
            ...
            startAngle: 270, endAngle: 270,
            pointers: <GaugePointer>[
              MarkerPointer(value: 2,
                 borderColor: const Color(0xFFFFCD60),
                borderWidth:3, color: Colors.white,
                markerHeight: 15,
                markerWidth: 15, markerType: MarkerType.circle,
              ),
              MarkerPointer(value: 8,

                color: Colors.white,
                borderColor: const Color(0xFFFFCD60),
                markerHeight: 15,   borderWidth: 3,
                markerWidth:15, markerType: MarkerType.circle,
              ),
            ...
            ranges: <GaugeRange>[GaugeRange(endValue: _secondMarkerValue,
                sizeUnit: GaugeSizeUnit.factor,
                startValue: _firstMarkerValue, startWidth : 0.06,
                endWidth: 0.06)],
            annotations:<GaugeAnnotation>[
              GaugeAnnotation( widget: const Text('6 hr 40 m', style: TextStyle(fontSize: 18,
                  fontFamily: 'Times',fontWeight: FontWeight.bold,
                  color: Color(0xFF00A8B5)),),
                  positionFactor: 0.05, angle: 0
              )]
        )
      ]
  );
